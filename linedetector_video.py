...
#os.listdir("challenge_imgs/")
#os.listdir("solidWhiteRight_imgs/")
os.listdir("solidYellowLeft_imgs/")
...
def process_image(image):
    ...
    imshape = image.shape
    vertices = np.array([[(0,imshape[1]),(imshape[0]*5/16,imshape[1]*0.4), (imshape[0]*15/16, imshape[0]*9/16), (imshape[1],imshape[0])]], dtype=np.int32)
...
'''cap = cv2.VideoCapture('/home/lalo/DeteccionLineas/challenge.mp4') #solidWhiteRight
while(cap.isOpened()):
    ret, frame = cap.read()
    gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)    
    #cv2.imshow('frame',gray)    
    img=process_image(frame)
    cv2.imshow('efweokfvwer',img)
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break'''

#white_output = 'NewVideos/solidWhiteRightN.mp4'
#white_output = 'NewVideos/challengeN.mp4'
white_output = 'NewVideos/solidYellowLeftN.mp4'
...
c#lip1 = VideoFileClip("solidWhiteRight.mp4")
